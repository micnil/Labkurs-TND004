...
void Graph::mstPrim() const
{
    ...
    int s=1; //set the source to the first vertex!
    dist[s] = 0;
    ...
    int counter = 0;
    while (counter < size) { // while the whole list is not done
...
void Graph::mstKruskal() const
{
    ...
    for (int v=1; v<=size; ++v) {
        ...
        for(Node *u = array[v].getFirst(); u != nullptr; u=array[v].getNext()){
            if(u->vertex > v) { // to not insert the same edge again!
    ...
    while (counter < size - 1 ) { //h.empty() instead???
        ...
        if (d.find(v) != d.find(u)) { // do not belong to the same tree
            // print out stuff
            cout << " (" << setw(2) << v
            << ", " << setw(2) << u << ") ";
